...
var ddd = Math.floor((Math.random() * 5) + 3);
var eee = Math.floor((Math.random() * 5) + 3);
...
window.onload = function() {
    ...
    chapterSel.onchange = function() {
        ...
        for(var i = 1; i<31; i++)
        {
            temp = t0[i-1] + Math.floor((Math.random() * 0.1) + (-0.01));
            t0.push(temp);
            temp = t1[i-1] + Math.floor((Math.random() * 0.1) + (-0.01));
            t1.push(temp);
        }
        //var risk, rev_r, factor, temp3;
